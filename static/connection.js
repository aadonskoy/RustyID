...
function createDataChannel (){
    ...
    peerConnection.ondatachannel = function (ev) {
        ...
        ev.channel.onmessage = function(e){
            console.log("Request from ["+server+"]:" +e.data);
            $('body').append("<div>requested item: " + e.data +"</div>");
            dataChannel.send(getIdentityProperty(e.data));
        }
    };
...
function sendNegotiation(type, sdp){
    var json = { protocol: "one-to-one", from: client, to: server, action: type, data: sdp};
    ws.send(JSON.stringify(json));
    console.log("Sending ["+client+"] to ["+server+"]: " + JSON.stringify(sdp));
}

function processOffer(offer){
    var peerConnection = createDataChannel();
